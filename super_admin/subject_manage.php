...
<body>
  ...
  <div class="content-container">
    ...
    <div class="container my-5">
      ...
      <span class="badge badge-light">You can update subject details</span>
      <table class="data my-3 table-bordered table table-hover text-center">
        ...
        <tbody id="msg">
          ...
          $subject = "SELECT * FROM subject_register WHERE faculty_id=$faculty";
          $query = mysqli_query($conn, $subject);
          $count = 1;
          while ($row = mysqli_fetch_assoc($query)) {
            $department_id = $row['department_id'];

            $dep = mysqli_query($conn, "SELECT * FROM department WHERE `department_id`='$department_id'");
            $r = mysqli_fetch_assoc($dep);
    ...
    <th scope="row">' . $count . '</th>
      <td style="text-transform: uppercase;" data-target="subject_name">' . htmlspecialchars($row['subject_name']) . '
      </td>
      <td><label style="font-size: 12px; ">' . htmlspecialchars($row['year']) . '</label></td>

      <td><label style="font-size: 12px; ">' . $department_name . '</label>
      </td>
