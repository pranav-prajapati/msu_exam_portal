        ...
        <div class="container my-5">
            ...
            <table class="data my-3 table-bordered table table-hover text-center">
                ...
                <tbody>
                    ...
                    <?php
                    $count = 1;
                    $examiner = mysqli_query($conn, "SELECT * FROM subject_examiner_mapping");
                    while ($ex_row = mysqli_fetch_assoc($examiner)) {
                        $dep = $ex_row['department_id'];

                        $dep_name = mysqli_query($conn, "SELECT * FROM department WHERE `department_id`='$dep'");
                        $row = mysqli_fetch_assoc($dep_name);

                        echo '
                        
                        <tr>
                        <td>' . $count . '</td>
                        <td style="text-transform:uppercase; font-size:12px;">' . htmlspecialchars($row['department_name']) . '</td>
